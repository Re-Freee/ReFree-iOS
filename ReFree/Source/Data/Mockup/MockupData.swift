...
struct Mockup {
    ...
    static let detailRecipe: [Manual] = {
        return (0..<15).map { _ in
            return Manual(
                describe: "요리를 망치는 방법 중 세 가지를 설명하려 한다. 이것만 피해도 반은 갈 수 있다.. 첫 번째는 레시피에서 대충 비슷한 재료를 쓰는 것이고 두 번째는 양을 무시한 채 마음대로 넣는 것이다. 마지막 세 번째는 불조절을 무시한 채 만드는 것이다. 이를 염두하고 레시피를 잘 따라서 만들도록 하자~",
                imageURL: "RefreeLogo"
            )
        }
    }()
